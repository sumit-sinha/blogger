...
@Component({
  ...
  template: `
		<div class="well">
			...
			<blog-list *ngIf="searchContentData != null && searchContentData.isSearchContent"
        [blogList]="searchContentData" (onLinkClickError)="onLinkClickError()"></blog-list>
...
export class BlogSearchComponent {
  ...
  dataHelper: ApplicationDataHelper = ApplicationDataHelper.getInstance();
  ...
  onSearchBlogClick(userSearch: string) {
    this.filteredData = [];
    const searchedString = userSearch.toLowerCase();
    for (const key in this.searchedData.blogs) {
