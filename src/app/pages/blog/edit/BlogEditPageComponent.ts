...
export class BlogEditPageComponent {
  ...
  editorInitFunction(editor) {
    ...
    this.route.params.subscribe((params) => {
      if (params.blog) {
        const blogPageData = this.dataHelper.getPageData('blog')[params.blog],
  ...
  onSaveClick($event, type) {
    ...
    if (content == null || content.trim() === '') {
      this.showError({
        errors: [this.dataHelper.getLabel('tx_empty_blog_error')]
      });
      this.loading = null;
      return;
    }

    const contentInformation: any = this.processContent(content),
      parameters = {
        type: type,
   ...
   * @param args {Object}
   */
  private onSaveCallback(response: any, args: any) {
    ...
    try {
      ...
      json.errors = [this.dataHelper.getLabel('tx_response_error')];
    }
    ...
    if (json && json.success) {
      ...
      pageData[args.parameters.title] = {
        ...
        text: decodeURI(args.parameters.content),
   ...
   * @param args {Object}
   */
  private onSaveErrorCallback(response: any, args: any) {
  ...
  private processContent(content: string): Object {
    ...
    const length = contentIdDelimeters.length;
    for (let i = 0; i < length; i++) {
      const delimeter = contentIdDelimeters[i];
